...
def _GetMetainstallerPayloadFilenames(prefix,
  ...
  payload_files = [
      ...
      'KDSUpdateHelper.msi',
      'KDSUpdateBroker.exe',
      'KDSUpdateOnDemand.exe',
      'KDSUpdateComRegisterShell64.exe',
  ...
  if _IsSupportedOmaha2Version(omaha_version):
    payload_files.remove('KDSUpdateBroker.exe')
    payload_files.remove('KDSUpdateOnDemand.exe')
    payload_files.remove('KDSUpdateComRegisterShell64.exe')
  ...
  if (omaha_version[0] >= 1 and
    ...
    payload_files.append('KDSUpdateCore.exe')
